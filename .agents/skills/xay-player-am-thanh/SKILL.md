---
name: xay-player-am-thanh
description: Xây player âm thanh — dùng khi cần tạo thiết kế và code hướng dẫn cho audio player.
---

# Skill: Xây player âm thanh

## Purpose
Skill này giúp agent tạo thiết kế và code hướng dẫn cho audio player để phục vụ triển khai web nghe nhạc cá nhân.

## When to use
- Khi workflow gọi đúng tác vụ của skill này.
- Khi user yêu cầu đầu ra liên quan trực tiếp đến: tạo thiết kế và code hướng dẫn cho audio player.
...
|---|---|---|
| Nguồn chính | yêu cầu player, định dạng file, trạng thái UI, playlist | Rõ mục tiêu và không chứa secret nhạy cảm |
...
|---|---|---|---|---|
| D01 | Player component spec | Phục vụ bước kế tiếp | Markdown có bảng quyết định | Có input, output, tiêu chí pass |
| D02 | API contract | Hỗ trợ triển khai | Danh sách hành động | Có owner và điều kiện hoàn thành |
| D03 | test cases | Hỗ trợ QA | Checklist hoặc report | Có happy path và edge case |

## Output format/schema
```markdown
# Output — Xây player âm thanh
...
|---|---|---|
| Happy path | Tạo đầu ra cho tác vụ xây player âm thanh với input đầy đủ | Trả output đúng schema và có handoff |
