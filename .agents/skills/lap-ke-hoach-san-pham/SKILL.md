---
name: lap-ke-hoach-san-pham
description: Lập kế hoạch sản phẩm — dùng khi cần tạo backlog MVP, milestone và quyết định scope cho web nghe nhạc cá nhân.
---

# Skill: Lập kế hoạch sản phẩm

## Purpose
Skill này giúp agent tạo backlog MVP, milestone và quyết định scope cho web nghe nhạc cá nhân để phục vụ triển khai web nghe nhạc cá nhân.

## When to use
- Khi workflow gọi đúng tác vụ của skill này.
- Khi user yêu cầu đầu ra liên quan trực tiếp đến: tạo backlog MVP, milestone và quyết định scope cho web nghe nhạc cá nhân.
...
|---|---|---|
| Nguồn chính | mô tả sản phẩm, mục tiêu, ràng buộc, tiêu chí thành công | Rõ mục tiêu và không chứa secret nhạy cảm |
...
|---|---|---|---|---|
| D01 | MVP backlog | Phục vụ bước kế tiếp | Markdown có bảng quyết định | Có input, output, tiêu chí pass |
| D02 | milestone pilot | Hỗ trợ triển khai | Danh sách hành động | Có owner và điều kiện hoàn thành |
| D03 | scope gate brief | Hỗ trợ QA | Checklist hoặc report | Có happy path và edge case |

## Output format/schema
```markdown
# Output — Lập kế hoạch sản phẩm
...
|---|---|---|
| Happy path | Tạo đầu ra cho tác vụ lập kế hoạch sản phẩm với input đầy đủ | Trả output đúng schema và có handoff |
