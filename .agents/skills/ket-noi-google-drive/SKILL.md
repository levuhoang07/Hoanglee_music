---
name: ket-noi-google-drive
description: Kết nối Google Drive — dùng khi cần tạo kế hoạch và code tích hợp Google Picker/OAuth an toàn.
---

# Skill: Kết nối Google Drive

## Purpose
Skill này giúp agent tạo kế hoạch và code tích hợp Google Picker/OAuth an toàn để phục vụ triển khai web nghe nhạc cá nhân.

## When to use
- Khi workflow gọi đúng tác vụ của skill này.
- Khi user yêu cầu đầu ra liên quan trực tiếp đến: tạo kế hoạch và code tích hợp Google Picker/OAuth an toàn.
...
|---|---|---|
| Nguồn chính | OAuth config, mục tiêu Drive, player interface | Rõ mục tiêu và không chứa secret nhạy cảm |
...
|---|---|---|---|---|
| D01 | OAuth plan | Phục vụ bước kế tiếp | Markdown có bảng quyết định | Có input, output, tiêu chí pass |
| D02 | Picker flow | Hỗ trợ triển khai | Danh sách hành động | Có owner và điều kiện hoàn thành |
| D03 | security checklist | Hỗ trợ QA | Checklist hoặc report | Có happy path và edge case |

## Output format/schema
```markdown
# Output — Kết nối Google Drive
...
|---|---|---|
| Happy path | Tạo đầu ra cho tác vụ kết nối google drive với input đầy đủ | Trả output đúng schema và có handoff |
