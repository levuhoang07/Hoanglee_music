---
name: tao-visual-hoat-hoa
description: Tạo visual hoạt họa — dùng khi cần tạo prototype bot/nền chuyển động cho trạng thái đang phát nhạc.
---

# Skill: Tạo visual hoạt họa

## Purpose
Skill này giúp agent tạo prototype bot/nền chuyển động cho trạng thái đang phát nhạc để phục vụ triển khai web nghe nhạc cá nhân.

## When to use
- Khi workflow gọi đúng tác vụ của skill này.
- Khi user yêu cầu đầu ra liên quan trực tiếp đến: tạo prototype bot/nền chuyển động cho trạng thái đang phát nhạc.
...
|---|---|---|
| Nguồn chính | visual brief, style, performance target | Rõ mục tiêu và không chứa secret nhạy cảm |
...
|---|---|---|---|---|
| D01 | Visual spec | Phục vụ bước kế tiếp | Markdown có bảng quyết định | Có input, output, tiêu chí pass |
| D02 | state machine | Hỗ trợ triển khai | Danh sách hành động | Có owner và điều kiện hoàn thành |
| D03 | implementation notes | Hỗ trợ QA | Checklist hoặc report | Có happy path và edge case |

## Output format/schema
```markdown
# Output — Tạo visual hoạt họa
...
|---|---|---|
| Happy path | Tạo đầu ra cho tác vụ tạo visual hoạt họa với input đầy đủ | Trả output đúng schema và có handoff |
