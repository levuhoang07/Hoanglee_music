---
name: kiem-thu-web-nhac
description: Kiểm thử web nhạc — dùng khi cần tạo test plan, QA report và fix plan trước public.
---

# Skill: Kiểm thử web nhạc

## Purpose
Skill này giúp agent tạo test plan, QA report và fix plan trước public để phục vụ triển khai web nghe nhạc cá nhân.

## When to use
- Khi workflow gọi đúng tác vụ của skill này.
- Khi user yêu cầu đầu ra liên quan trực tiếp đến: tạo test plan, QA report và fix plan trước public.
...
|---|---|---|
| Nguồn chính | URL/source, checklist, file test, deploy target | Rõ mục tiêu và không chứa secret nhạy cảm |
...
|---|---|---|---|---|
| D01 | Test plan | Phục vụ bước kế tiếp | Markdown có bảng quyết định | Có input, output, tiêu chí pass |
| D02 | QA report | Hỗ trợ triển khai | Danh sách hành động | Có owner và điều kiện hoàn thành |
| D03 | public decision brief | Hỗ trợ QA | Checklist hoặc report | Có happy path và edge case |

## Output format/schema
```markdown
# Output — Kiểm thử web nhạc
...
|---|---|---|
| Happy path | Tạo đầu ra cho tác vụ kiểm thử web nhạc với input đầy đủ | Trả output đúng schema và có handoff |
